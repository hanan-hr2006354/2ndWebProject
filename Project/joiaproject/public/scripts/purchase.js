...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (!loggedInUser || !productId) {
      alert('Missing required information. Please log in or select a valid product.');
    ...
    function populateProductDetails(product) {
      ...
      productDetailsDiv.innerHTML = `
        <img src="/images/${product.image}" alt="Item Image">
        <h4 class="name">Product Name: ${product.name}</h4>
        <h4 class="price">Product Price: ${product.price}</h4>
        <label for="quantity">Quantity:</label>
        <input type="number" id="quantity" name="quantity" required min="1" max="${product.quantity}">
      ...
      document.getElementById('purchase-now-btn').addEventListener('click', async function () {
        const quantityChosen = parseInt(document.getElementById('quantity').value);
        const totalPrice = product.price * quantityChosen;
        const shippingAddress = document.getElementById('shipping_address').value;
  
        if (quantityChosen > product.quantity) {
        ...
        try {
          ...
          if (!purchaseResponse.ok) {
            throw new Error(`Error: ${purchaseResponse.statusText}`);
          }

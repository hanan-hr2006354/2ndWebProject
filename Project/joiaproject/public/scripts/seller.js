...
document.addEventListener('DOMContentLoaded', async function() {
    try {
      ...
      populateSellerProducts(products);
    } catch (error) {
      console.error('Error fetching seller products:', error);
      alert('Failed to load seller products. Please try again.');
    }
  
    function populateSellerProducts(products) {
      const listProductHTML = document.querySelector('.seller-category');
      products.forEach(product => {
        ...
        newProduct.innerHTML = `
          <div class="card">
            <img src="/images/${product.image}" width="250" height="300">
            <p>${product.name}</p>
